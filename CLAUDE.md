...
- **GTK4 list rows select on RELEASE; this app selects on the PRESS**
  (D37): `on_app_select_on_press` (app.h) on every cell/row child runs
  `list.select-item` at press time and claims the release so GTK's own
  selection cannot run a second time.  Never install it on a widget that
  contains a gesture of its own (the claim cancels everything below it):
  the sidebar has it on the expander's label, the check cell has none.
