 ...
 *
 * Both message texts are GTK3's (GTK4 has no GdkAtom at all, and its menu
 * tracker is the same code under the same MacPorts patch); they are kept
 * as-is across the port since a filter that never matches costs nothing,
 * and the GTK4 texts, if any, are re-measured in the dev sandbox.
